...
import {
  ...
  type ConnectorActionCapabilitiesIndex,
  getConnectorActionCapabilities,
} from '../../../shared/lib/action_type_utils';
import type { YamlValidationResult } from '../model/types';

const validateStep = (
  step: StepInfo,
  capabilityIndex: ConnectorActionCapabilitiesIndex,
  ...
  const capabilities = getConnectorActionCapabilities(connectorId, capabilityIndex);
  ...
  return {
    ...
    ruleId: 'unsupportedConnectorAction',
    severity: 'warning',
...
export function validateUnavailableConnectorActions(
  ...
  const capabilityIndex = buildConnectorActionCapabilitiesIndex(connectorTypes);
  return Object.values(workflowLookup.steps).flatMap((step) => {
    const result = validateStep(step, capabilityIndex, lineCounter);
