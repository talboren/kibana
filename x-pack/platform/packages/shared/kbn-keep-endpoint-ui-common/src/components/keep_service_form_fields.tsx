...
import { ConfigEntryView, KeepProvider } from '../types/types';
...
export const KeepServiceFormFields: React.FC<KeepServicesProps> = ({
  ...
  const onProviderChange = useCallback(
    (provider?: string) => {
      ...
      const inferenceId = generateInferenceEndpointId({
        ...
        taskType: Object.keys(newProvider?.config ?? {})[0] ?? DEFAULT_TASK_TYPE,
      });
  ...
  useEffect(() => {
    if (providers) {
      ...
      const elasticServiceIndex = providers.findIndex((provider) => provider.type === 'elastic');
  ...
  useEffect(() => {
    if (config?.provider && config?.taskType && isEdit) {
      const newProvider = updatedProviders?.find((p) => p.id === config.provider);
